...
def init_db():
    con = sqlite3.connect("settings.db")
    cur = con.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS ChannelSettings (channel_id TEXT PRIMARY KEY, guild_id TEXT, isSlowed INTEGER, activeSlow INTEGER, slowTime INTEGER, embedSlow INTEGER, slowMessage TEXT)")
